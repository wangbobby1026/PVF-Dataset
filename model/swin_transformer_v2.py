...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 10, 'input_size': (3, 112, 112), 'pool_size': (8, 8),
...
@register_model
def swinv2_tiny_window7_224(pretrained=False, **kwargs) -> SwinTransformerV2:
    """
    """
    model_args = dict(window_size=7, embed_dim=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24))
    return _create_swin_transformer_v2(
        'swinv2_tiny_window7_224', pretrained=pretrained, **dict(model_args, **kwargs))
